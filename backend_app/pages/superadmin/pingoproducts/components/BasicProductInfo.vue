  ...
  methods: {
    ...
    update_product() {
      ...
      if (check_result !== "") {
        ...
        delete this.product.item_pingo_sliderimages;
        delete this.product.variations;

        var vm = this;
        console.log("before update", this.product)
        this.$store.dispatch("pingoproducts/superadmin_update_product", this.product)
          .then((response) => {
            console.log("aftr update",response)
            if (response.id>0) {
              vm.product.id = response.id;
              vm.product.supplier = response.supplier.id;
              vm.product.category = response.category;
              Swal.fire("Success", "情報更新されました！", "success")
              vm.$emit("optResult", {result: true, product: response})
            }
          }).catch(error => {
          console.log("update error message",error.response.data)
          Swal.fire({
            icon: "error",
        ...
        this.$store.dispatch("pingoproducts/superadmin_create_product", this.product)
          .then((response) => {
            ...
            if (response.pingo_item && response.pingo_item.id) {
              vm.product = response.pingo_item;
              vm.product.supplier = response.pingo_item.supplier.id;
              vm.product.category = response.pingo_item.category;
              Swal.fire("Success", "Product was created!", "success")
              vm.$emit("optResult", {result: true, product: response.pingo_item})
            }
          })
      }
    },
    ...
    <div class="card-body">
      ...
      <div>
        ...
        <div class="form-group mt-3 float-right">
          <b-button class="btn-rounded" variant="warning" @click="update_product" v-if="product.id">更新
          </b-button>
