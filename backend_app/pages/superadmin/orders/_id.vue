...
export default {
  ...
  components: {
    ...
    modalUpdateSupplierPayment: () =>
      import("../components/modalUpdateSupplierPayment"),
  ...
  async asyncData({ params }) {
    ...
    if (order_id) {
      ...
      let orderDetail = await orderService
        .get_list(options)
        .then(response => {
          return response;
        })
      ...
      let margins = await marginService
        .load_list(`?expand=user&filter{from_orderID}=${order_id}`)
        .then(response => {
  ...
  methods: {
    ...
    marginUpdated(new_margin) {
      let self = this;
      let marginIndex = self.margins.findIndex(
        margin => margin.id === new_margin.id
      );
      if (marginIndex > -1) {
    ...
    async updateOrderItemStatus(orderitem_id) {
      ...
      if (status) {
        orderService
          .updateOrderItemStatus({ orderitem_id: orderitem_id, status: status })
          .then(res => {
            console.log("update item status", res);
            if (parseInt(res.orderitem_id)) {
              let orderitemIndex = self.order.orderitems.findIndex(
                item => item.id === orderitem_id
              );
              if (orderitemIndex > -1) {
              ...
              swalService.showModal(
                `Item Status Update! `,
                `orderitem #${orderitem_id}  status: ${status} updated!`,
                "success"
              );
            }
          });
    ...
    UpdateOrderItem(response) {
      ...
      if (response.result) {
        ...
        let itemIndex = this.order.orderitems.findIndex(
        ...
        if (itemIndex > -1) {
          ...
          swalService.showToast(
            "success",
            `Item #${newItem.id} delivery info has beedn updated!`
          );
        }
      }
    ...
    UpdateOrderItemResult(info) {
      ...
      if (info.result) {
        new_orderitems.forEach(newitem => {
          let itemIndex = self.order.orderitems.findIndex(
            item => item.id === newitem.id
          );
          if (itemIndex > -1) {
            self.order.orderitems[itemIndex].delivered = newitem.delivered;
            self.order.orderitems[itemIndex].delivered_at =
              newitem.delivered_at;
            self.order.orderitems[itemIndex].delivery_info =
              newitem.delivery_info;
...
<template>
  <div>
          ...
          <div class="card-header border-bottom bg-transparent">
            <h5 class="header-title mb-0">
              ...
              <span
                class="ml-md-3 badge"
                :class="{
                  'badge-soft-danger': order.status === 'NEW',
          ...
          <div class="card-body">
            <div>
              ...
              <b-alert
                show
                dismissible
                class="bg-danger text-white border-0 mb-2"
                v-if="order.message !== ''"
              >
                {{ order.message }}
              </b-alert>
                ...
                <div class="col-lg-3 col-sm-6">
                  <div class="media mb-2">
                    ...
                    <div class="media-body">
                      <p class="mb-1">日付</p>
                      <h5 class="mt-0">
                        {{ order.ordered_at | short_date }}
                        <small class="text-muted">{{
                          order.ordered_at | short_time
                        }}</small>
                      </h5>
                    </div>
                  </div>
    ...
    <DeliveryModal
      :openDeliveryModal="showmodel_delivery"
      :orderitem_ids="multipleSelection"
      :orderitem="edit_orderitem"
      @updateResult="UpdateOrderItemResult"
    ></DeliveryModal>

    <modalUpdateSupplierPayment
      :showModal="showmodel_paysupplier"
      :orderitem_ids="multipleSelection"
      :orderitem="edit_orderitem"
      @updateResult="UpdateOrderItemResult"
    ></modalUpdateSupplierPayment>
