...
class NoAccessPermission(APIException):
    status_code = status.HTTP_403_FORBIDDEN
    default_detail = "No permission to execute operation."
    default_code = "no_access_permission"


def get_response(message="", result=False, status_code=200):
    return {
        "message": message,
        "result": result,
...
class ExceptionMiddleware(object):
    ...
    def __call__(self, request):
        ...
        if response.status_code == 403 and "No permission" in str(response.content):
            response = get_response(
                message="No Permission to endpoints",
                status_code=response.status_code
            )
            return JsonResponse(response, status=response['status_code'])
