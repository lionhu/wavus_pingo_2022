      ...
      <div class="nichiei-container row" v-show="formLoaded">
        <div id="sq-ccbox col-12" class="position-relative">
          <h4 class="text-white">{{ $t('payment_creditcard.charge_amount') }}
            <span class="ml-3">{{ chargeAmount|currency("¥") }}</span>
          </h4>
...
export default {
  name: "paymentModal",
  props: ["openPaymentModal", "message"],
  data: function () {
    return {
      errors: [],
      postcode: "3360031",
  ...
  computed: {
    ...mapGetters({
      ...
      shippingaddress: 'addressbook/getterOrderAddress',
    }),
    chargeAmount: function () {
      if (this.cart === null) return 0;
      return this.cart.quantity * this.cart.product.discount_price;
    }
  },
  methods: {
    load_order() {
        ...
        callbacks: {
          ...
          cardNonceResponseReceived: function (errors, nonce, cardData) {
            ...
            var new_orderdata = {
              nonce: nonce,
              payment_method: "CARD",
              product: vm.cart.product.id,
              json_product: vm.cart.product,
              point_usage: {"use_point": 0, "apply_point": false},
              quantity: parseInt(vm.cart.quantity),
              message: vm.message,
              json_shippingaddress: vm.shippingaddress,
              chargeAmount: parseInt(vm.chargeAmount),
              price:vm.cart.product.discount_price,
              total:parseInt(vm.chargeAmount),
            };

            console.log(new_orderdata)
            vm.$store.dispatch("orders/placePingoOrder", new_orderdata)
                ...
                if (response.result) {
                  ...
                  Swal.fire({
                    ...
                    if (result.isConfirmed) {
                      vm.$store.commit("authfack/setPointBalance", response.data);
                    ...
                    icon: 'warning',
                    title: '支払いは未完了',
                    html: '注文の支払いは何らかの原因で完了できませんでした。後ほど再度試してみてくだい。<br> または、弊社にお問い合わせください。',
                    footer:"お問合せ先： crs@wavus.jp"
                  })
                  })
                }
              });
