          ...
          <div class="col-lg-6 offset-lg-3">
            ...
            <div class="theme-form">
              ...
              <div class="form-row">
                ...
                <button class="btn-solid btn" @click="onSubmit" :disabled="invalid"> {{
...
export default {
  ...
  methods: {
        ...
        if (error.data.error_code === "L03") {
          Swal.fire({
            ...
            preConfirm: (login) => {
              ...
              return userService.resend_activation({email: this.email})
                .then(response => {
    ...
    onSubmit() {
      APIServices.post('auth/users/reset_password/', {"email": this.email})
        .then(resp => {
          if (resp.result) {
            swalService.showToast("success", "認証メールも送付いたしましたので、ご確認ください。")
            this.$router.push('/')
          } else {
            swalService.showToast("error", "システムエラー。管理者（crs@wavus.jp）までご連絡ください。")
          }
        })
    },
  },
